...
// Historical PTY output is display data, not a script that should reconfigure the
// live xterm instance. Strip sequences that make xterm send bytes back to the new
// shell (mouse/focus/paste modes and terminal capability queries).
const SCROLLBACK_SIDE_EFFECT_SEQ =
  /\x1b\[(?:\?(?:1|9|1000|1001|1002|1003|1004|1005|1006|1015|1016|2004|2026)[hl]|\??(?:5|6)n|[=>?]?[0-9;]*c|\?[0-9;]+\$p|(?:14|16|18)t)/g

const SCROLLBACK_MODE_RESET =
  ...
  '\x1b[?2004l\x1b[?2026l\x1b[?1l\x1b[m\x1b[?25h\x1b[2J\x1b[H'
...
export function preFillTerminal(sessionId: string, rawBase64: string): void {
  ...
  const cleaned = raw.replace(SCROLLBACK_SIDE_EFFECT_SEQ, '')
  // Keep replay and cleanup in one parser write. This prevents DOM events from
  // firing during a gap where historical mouse tracking is enabled.
  const replay = `${cleaned}${SCROLLBACK_MODE_RESET}`
  const bytes = Uint8Array.from(replay, (c) => c.charCodeAt(0))
  term.write(bytes)
  ...
  //   \x1b[2J/H    — clear only the visible viewport; restored history remains
  //                  available above in scrollback instead of looking like a
...
/** [2026-07-31] 代理模式中断只终止结构化 Claude 子进程，不向隐藏 PTY 注入控制码。 */
export function cancelEmbedAgentRun(sessionId: string): void {
  void window.electronAPI.agentCancel(sessionId).then(() => {
    useEmbedAwaitingReplyStore.getState().clearPending(sessionId)
  })
}

export function useXTerminal({ sessionId, active }: Props) {
  ...
  const onContextMenu = (_e: MouseEvent): void => {
  ...
  return {
    ...
    style: {
      ...
      /* [2026-07-08] 高度由 ResizeObserver 测量父元素并设置为 inline style。 */
    },
    onContextMenu
  }
}
