...
const PLUGIN_ZH: Record<string, string> = {
  ...
  'intercom': 'Intercom 客服集成：搜索对话、分析客服模式、查找联系人',
  'postman': 'Postman API 管理：同步集合、生成客户端代码、发现 API、运行测试、创建 mock、发布文档、安全审计',
  'postiz': '社交媒体自动化：28+ 平台发帖调度、集成管理、媒体上传、分析追踪',
  'discord': 'Discord 消息桥：内置访问控制，/discord:access 管理',
  'telegram': 'Telegram 消息桥：内置访问控制，/telegram:access 管理',
  'imessage': 'iMessage 消息桥：读取 chat.db，AppleScript 发送',
  'firebase': 'Firebase MCP：Firestore、认证、云函数、托管、存储',
  'gitlab': 'GitLab DevOps：仓库、MR、CI/CD、issue、wiki',
  'linear': 'Linear 项目管理：issue、项目、状态、搜索',
  'asana': 'Asana 任务管理：创建任务、搜索项目、跟踪进度',
  'circleback': 'Circleback 上下文：会议、邮件、日历事件',
  'sentry': 'Sentry 错误监控：错误报告、堆栈追踪',
  'atlassian': 'Atlassian 集成：Jira + Confluence',
  'miro': 'Miro 白板：画板访问、创建图表',
  'figma': 'Figma 设计：设计文件、组件信息',
  'zapier': 'Zapier 自动化：8000+ 应用',
  'spotify-ads-api': 'Spotify 广告管理',
  'claude-hud': '终端 HUD 状态栏：实时显示上下文用量、活跃工具、Git 信息',
}
