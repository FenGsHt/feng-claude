...
### 修复
- **触发器在经典终端中恢复执行**：倒计时、定时和重复任务会直接写入当前 Claude Code 终端；消息代理会话仍走独立网关，Claude Code 的 TUI 界面不再阻止触发器投递
...
### 修复
- **macOS 调试浏览器关闭布局恢复**：关闭内嵌调试浏览器会同步通知当前所有应用窗口；从程序坞恢复窗口后，右侧不再残留黑色空白区域
...
### 修复
- **Finder 文件拖入恢复**：兼容 Electron 43 的官方文件路径接口；macOS 下从 Finder 拖入终端、复制文件后 `Command+V`、以及消息代理输入框粘贴文件，都会正确写入 Claude 可读取的 `@路径` 引用
...
### 修复
- **macOS 应用内下载可正常完成**：修复 `content-length` 被误读为单个字符的问题；完整的 DMG 不再因显示为 `…/1 字节` 而被错误删除
- **进度显示可信**：下载总大小缺失时显示不确定进度，已知大小时百分比限制在 0–100%
...
### 修复
- **macOS 会话创建恢复正常**：应用获准打开后，终端运行组件会复制到应用数据目录再启动，避免 Gatekeeper 单独拦截 `node-pty` 的内部辅助程序
- **失败自动回退内嵌终端**：iTerm2 或持久终端守护进程无法启动时，会直接使用软件内嵌终端，不再出现点击创建/重启会话无反应
- **启动错误即时可见**：PTY daemon 崩溃会立即返回真实原因并留下诊断日志，不再只显示等待超时
...
### 新功能
- **消息代理模式**：外嵌聊天改用 Claude Code 结构化 JSON 流，每个会话独立排队、保持上下文并支持中断，不再模拟终端键盘或解析控制码
- **macOS 应用内更新**：可在软件内下载与本机架构匹配的 DMG、查看实时进度，并在下载完成后自动打开安装包

### 修复
- **Dock 退出恢复正常**：非阻塞关闭 PTY 与后台服务，修复右键「退出」后应用仍驻留、必须强制退出的问题
- **外嵌回复稳定性**：统一网关运行状态和流式气泡合并，减少等待状态、重复消息及终端 TUI 干扰
...
### 修复
- **macOS 首次启动可手动放行**：安装包现在带有完整的 ad-hoc 签名；首次打开被拦截后，可在「系统设置 → 隐私与安全性」中点击「仍要打开」，不再提示应用已损坏，也无需执行 `xattr`
- **阻止异常安装包发布**：打包阶段会校验应用本体、Electron Framework 与所有 Helper 的签名完整性，校验失败则终止构建
...
### 修复
- **macOS 更新不再无限等待**：发现新版后显示适合当前 Mac 架构的 DMG 下载按钮，不再对未签名版本执行不可靠的后台自动安装
- **更新包链接一致**：DMG、ZIP 和更新元数据统一使用无空格文件名，修复 GitHub Release 资源 404
- **错误诊断增强**：更新失败原因会保留在界面中，并记录到本地 `update.log`
