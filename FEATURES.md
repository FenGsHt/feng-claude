...
- **拖拽文件到终端**：自动生成 `@` 路径引用
- **显示与资源管理器一致**：dot 文件夹（`.creator`/`.history` 等）及 `build`/`dist`/`temp`/`library` 等均显示，仅隐藏 `node_modules`/`.git`/`.svn`/`.hg`
...
- **实例内**：同一窗口里只有一个会话能持有 bot，其它会话用隔离目录
- **跨实例**：多窗口=多个独立 app 实例，用 `feng-owner.lock`（记录持有的主进程 pid）保证同一 token 全局只有一个实例轮询，其它实例的会话自动退避；持有实例退出/崩溃后锁自动失效可接管
- **强制重连夺锁**：在哪个窗口点 `↻`，哪个窗口就成为新 owner，消息从此只投递到该窗口
- 共同防止多个 server.ts 争抢同一 bot 的 getUpdates（409 Conflict / -32000 / 消息投错窗口）
...
- **纳入 Claude 回答**：点评时把 Claude 最近一条回答的摘要带入上下文，建议贴合真实发生的事，而非泛泛而谈
- **报错哨兵**：会话以错误状态结束时无视概率强制触发、冷却减半，优先给排错方向
- **建议可一键执行**：回复中用反引号包裹的命令会在气泡里显示「▶ 命令」按钮，点击填入当前终端（不自动回车）
- **成本/Git 哨兵**：单轮输出 token 偏多时提示留意成本；未提交改动较多时提醒可 commit
- 输入采集会剥离鼠标跟踪等转义序列，避免噪声污染宠物上下文
